...
} catch(*) { setImgSrc(dataUrl); setImgB64(dataUrl.split(”,”)[1]); }
};
image.src = dataUrl;
} catch(*) { setImgSrc(dataUrl); setImgB64(dataUrl.split(”,”)[1]); }
};
...
setErr(“AI features not configured. Admin: set VITE_ANTHROPIC_API_KEY in Vercel env vars.”);
return;
}
...
```
  ...
  const r1 = await fetch("https://api.anthropic.com/v1/messages", {
    method:"POST",
    headers:{"x-api-key":ANTHROPIC_API_KEY,"anthropic-dangerous-direct-browser-access":"true","anthropic-version":"2023-06-01","content-type":"application/json"},
    body: JSON.stringify({
...
if (!r2.ok) { let m=“Step 2 failed: HTTP “+r2.status; try{m=JSON.parse(raw2).error?.message||m;}catch(*){} throw new Error(m); }
const env2 = JSON.parse(raw2);
const txt2 = (env2.content||[]).filter(b=>b.type===“text”).map(b=>b.text||””).join(””).trim();
let web = {};
try { const s2=txt2.lastIndexOf(”{”), e2=txt2.lastIndexOf(”}”); if(s2>=0&&e2>s2) web=JSON.parse(txt2.slice(s2,e2+1)); } catch(*) {}
...
<link href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"/>
<style>{`@keyframes pulse{0%,100%{opacity:.4}50%{opacity:1}} input:focus,select:focus,textarea:focus{border-color:${P.accent}!important;box-shadow:0 0 0 3px rgba(74,143,219,.15)} button:hover:not(:disabled){filter:brightness(1.1)} tr:hover{background:rgba(74,143,219,.05)!important} `}</style>
